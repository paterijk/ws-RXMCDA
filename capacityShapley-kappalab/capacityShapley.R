# usage:
# R --slave --vanilla --args "[inDirectory]" "[outDirectory]" < capacityShapley.R
# example: 
#  R --slave --vanilla --args "${PWD}/capacityShapleyInFiles" "${PWD}/capacityShapleyOutFiles" < capacityShapley.R
...
if (is.null(errFile)){
	...
	if (is.null(errData))
	{
		...
		tmpErr<-try(
				{
					shap<-Shapley.value(m)
					critVals<-c()
					for (i in 1:length(shap)){
						critVals<-rbind(critVals,c(i,shap[i]))
					}
				}
		)
		
		if (inherits(tmpErr, 'try-error')){
			errCalc<-"Cannot calculate Shapley values of given capacity."
		} else {
			execFlag<-TRUE
		}
	}
...
if (execFlag){
	...
	status<-putCriteriaValues(outTree, critVals, critIDs, "shapleyValues")
	
	status<-saveXML(outTree, file="shapleyValues.xml")
