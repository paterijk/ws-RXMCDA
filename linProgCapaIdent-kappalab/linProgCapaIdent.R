# usage:
# R --slave --vanilla --args "[inDirectory]" "[outDirectory]" < linProgCapaIdent.R
# example: 
#  R --slave --vanilla --args "${PWD}/linProgCapaIdentInFiles" "${PWD}/linProgCapaIdentOutFiles" < linProgCapaIdent.R
...
if (is.null(errFile)){
	...
	if (flag){
		if ((k>=1)&(k<=n))
		{				
			alternativesComparisons<-getAlternativesComparisons(treeAlternativesPreorder, perfTable)
		}
		else
		{
	...
	if (flag){
		if (alternativesComparisons$status == "OK") 
		{
			choquetPreorder<-alternativesComparisons[[1]]
			if (is.null(choquetPreorder))
			{
	...
	tmpErr<-try(
			{
				output<-lin.prog.capa.ident(n,
						k,
						A.Choquet.preorder=choquetPreorder,
