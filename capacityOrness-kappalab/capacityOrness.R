# usage:
# R --slave --vanilla --args "[inDirectory]" "[outDirectory]" < capacityOrness.R
# example: 
# R --slave --vanilla --args "${PWD}/capacityOrnessInFiles" "${PWD}/capacityOrnessOutFiles" < capacityOrness.R
...
if (is.null(errFile)){
	...
	if (is.null(errData))
	{
		...
		if (inherits(tmpErr, 'try-error')){
			errCalc<-"Cannot calculate orness of given capacity."
		} else {
			execFlag<-TRUE
		}
	}
...
if (execFlag){
	...
	status<-putCriterionValue(outTree, orn, criteriaIDs = critIDs, mcdaConcept = "capacityOrness")
	
	status<-saveXML(outTree, file="capacityOrness.xml")
