# usage:
# R --slave --vanilla --args "[inDirectory]" "[outDirectory]" < capacityFavorValues.R
# example: 
#  R --slave --vanilla --args "${PWD}/capacityFavorInFiles" "${PWD}/capacityFavorOutFiles" < capacityFavor.R
		...
		tmpErr<-try(
				{
					...
					for (i in 1:length(fav)){
						critVals<-rbind(critVals,c(i,fav[i]))
					}
				}
		)
		
		if (inherits(tmpErr, 'try-error')){
			errCalc<-"Cannot calculate favor values of given capacity."
		} else {
			execFlag<-TRUE
		}
	}
...
if (execFlag){
	...
	status<-putCriteriaValues(outTree, critVals, critIDs, "favorValues")
	
	status<-saveXML(outTree, file="favorValues.xml")
