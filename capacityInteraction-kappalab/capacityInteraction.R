# usage:
# R --slave --vanilla --args "[inDirectory]" "[outDirectory]" < capacityInteraction.R
# example: 
#  R --slave --vanilla --args "${PWD}/capacityInteractionInFiles" "${PWD}/capacityInteractionOutFiles" < capacityInteraction.R
...
if (is.null(errFile)){
	...
	if (is.null(errData))
	{
		...
		tmpErr<-try(
				{
					inter<-interaction.indices(m)
					critVals<-c()
					for (i in 1:dim(inter)[1]){
						for (j in i:dim(inter)[2]){
							if (i!=j) critVals<-rbind(critVals,c(i,j,inter[i,j]))	
						}
					}
				}
		)
		...
		if (inherits(tmpErr, 'try-error')){
			errCalc<-"Cannot calculate interaction values on given capacity."
		} else {
			execFlag<-TRUE
		}
	}
...
if (execFlag){
	...
	status<-putCriteriaPairsValues(outTree, critVals, critIDs, "interactionValues")
	
	status<-saveXML(outTree, file="interactionValues.xml")
