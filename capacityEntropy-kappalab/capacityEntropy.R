# usage:
# R --slave --vanilla --args "[inDirectory]" "[outDirectory]" < capacityEntropy.R
# example: 
# R --slave --vanilla --args "${PWD}/capacityEntropyInFiles" "${PWD}/capacityEntropyOutFiles" < capacityEntropy.R
...
if (is.null(errFile)){
	...
	if (is.null(errData))
	{
		...
		if (inherits(tmpErr, 'try-error')){
			errCalc<-"Cannot calculate entropy of given capacity."
		} else {
			execFlag<-TRUE
		}
	}
...
if (execFlag){
	...
	status<-putCriterionValue(outTree, ent, criteriaIDs = critIDs, mcdaConcept = "capacityEntropy")
	
	status<-saveXML(outTree, file="capacityEntropy.xml")
