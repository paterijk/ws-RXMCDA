# usage:
# R --slave --vanilla --args "[inDirectory]" "[outDirectory]" < generateNormalizedPerformanceTable.R
# example: 
# R --slave --vanilla --args "${PWD}/in" "${PWD}/out" < generateNormalizedPerformanceTable.R
...
if (is.null(errFile)){
	...
	if (is.null(errData))
	{
		...
		if (inherits(tmpErr, 'try-error')){
			errCalc<-"Cannot generate performance table."
		} else {
			execFlag<-TRUE
		}
	}
...
if (execFlag){
	...
	status<-putPerformanceTable(outTree, M, "performanceTable")
	
	status<-saveXML(outTree, file="performanceTable.xml")
