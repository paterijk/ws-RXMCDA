# usage:
# R --slave --vanilla --args "[inDirectory]" "[outDirectory]" < capacityVariance.R
# example: 
#  R --slave --vanilla --args "${PWD}/capacityVarianceInFiles" "${PWD}/capacityVarianceOutFiles" < capacityVariance.R
...
if (is.null(errFile)){
	...
	if (is.null(errData))
	{
		...
		if (inherits(tmpErr, 'try-error')){
			errCalc<-"Cannot calculate variance of given capacity."
		} else {
			execFlag<-TRUE
		}
	}
...
if (execFlag){
	...
	status<-putCriterionValue(outTree, var, criteriaIDs = critIDs, mcdaConcept = "capacityVariance")
	
	status<-saveXML(outTree, file="capacityVariance.xml")
