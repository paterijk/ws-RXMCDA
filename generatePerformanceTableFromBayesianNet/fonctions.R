...
##################################################################################################
##############################################TU DOIS CHANGER ICI PATRICK#########################	
##################################################################################################
		
					quadratic<-tX%*%bloc%*%X
					# print(quadratic)
					coef.ajustment<-sqrt(1/quadratic)
					
					if(quadratic<1)####Si la forme quadratique ets bonne , on parametrise#################
					{
								state.parametrisation<-TRUE	
								message.error<-"Parametrization succesfull."
								Newborne1<-NA
								Newborne2<-NA
					...
					else ##Dans ces cas une erreus est produite et il faut detecter la mauvaise corr?lation et propos? des bornes######
					{
							...
							message.error<-"Parametrization unsuccessful."
							...
							borne2.Bloc<-(1/sqrt(mat.adjency.topo[num.cor,num.cor]))*(-(at.Bloc%*%y)-sqrt(rho))/(ax.Bloc)
							K<-max(abs(borne1),abs(borne2))
...
return(list(List.nodes=list.nodes,Mat.adjency.topo=mat.adjency.topo,State.parametrisation=state.parametrisation,Correlation.error=correlation.error,Borne.correlation=c(Newborne1,Newborne2),Message.error=message.error,Coef.ajustement=coef.ajustment,tX=tX,bloc=bloc,X=X))
